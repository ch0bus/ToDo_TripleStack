...
Системные теги (`user=null`) + теги текущего пользователя. Скрытие системных тегов в UI — только `localStorage` клиента; API по-прежнему отдаёт полный список.
...
Быстрый вход (PIN, WebAuthn) хранится в `localStorage` клиента и не ходит на сервер. Скрытие системных тегов — тоже локальная настройка устройства.

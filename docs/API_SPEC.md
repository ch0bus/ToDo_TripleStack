...
Быстрый вход (PIN, WebAuthn) хранится в `localStorage` клиента и не ходит на сервер.
...
События месяца на `/calendar`: список «События месяца», query `esort` (`start`, `new`, `title`) и `ecolor` (цвет). Сетка и выбранный день от этих параметров не зависят. На входящих события в блоке выбранного дня (`?day=YYYY-MM-DD`, без параметра — сегодня). Просрочено / Все / Готово от выбранного дня не зависят.

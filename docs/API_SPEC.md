...
Одна заметка на календарный день у пользователя. День с заметкой в UI: золотой уголок на месяце, золотая полоска под числом на году. Сегодня — золотая рамка плитки (месяц) или нимб как в логотипе (год).
...
Именованный факт во времени (встреча, день рождения, отпуск). Не задача: без статуса, приоритета, срока и подзадач. Во входящих показывается только в «Сегодня».
...
**color:** `#RRGGBB`, закладка на месяце / квадрат на годе.
...
Сортировка списков — query `sort` (`new`, `created`, `due`, `priority`): на входящих только «Все задачи» (по умолчанию новые сверху), в календаре только «Задачи месяца» (по умолчанию по сроку). API всегда отдаёт `-created_at`.
События месяца на `/calendar`: список «События месяца», query `esort` (`start`, `new`, `title`) и `ecolor` (цвет). Сетка и выбранный день от этих параметров не зависят. На входящих события только в блоке «Сегодня».

Календарь (`/calendar`) грузит свои `/todos/`, `/events/`, `/shift-days/` и `/day-notes/` на видимый месяц или год.
Доска смен выбирается `?calendar=id`. Сводка смен выбранного дня — блок с меню: настройки на `/settings/shifts`, рисование открывает кисть под сеткой. Задачи ставятся в день `event_date`, иначе `due_date`. События — по `start_at`…`end_at`. Смены двух слоёв заливают день по диагонали.
День с заметкой — золотой уголок (месяц) или полоска под числом (год). Сегодня — золотая рамка плитки (месяц) или нимб как в логотипе (год). День с событием — закладка в верхнем правом углу (месяц) или квадрат (год).

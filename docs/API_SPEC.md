...
**Повторение:** правило `daily` / `weekly` / `monthly` задаёт шаг внутри окна
от `created_at` до `due_date` (включительно). Новая копия при «Готово» не создаётся.
Без срока повторы на календаре не показываются.

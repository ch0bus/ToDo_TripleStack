...
Один шаблон-цикл на аккаунт разворачивается с `start_date` до `end_date` (пусто — без конца).
Ручная метка дня перекрывает шаблон и действует и вне окна цикла.

### GET / POST `/api/shift-kinds/`

- `GET` — типы смен текущего пользователя
- `POST` — `{ "name": "Ночь", "color": "#7c3aed", "duration_hours": "11.00", "break_minutes": 60, "hourly_rate": "250.00" }`
...
`end_date: null` — цикл без конца. `kind_id: null` — выходной в цикле. Пустой `slots` — только ручные метки.

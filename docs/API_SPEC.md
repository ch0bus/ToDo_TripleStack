...
- Данные **персональные**: todos, личные теги, подзадачи и смены фильтруются по пользователю из JWT.
...
- шаг `daily` / `weekly` / `monthly` внутри окна от календарного дня `created_at` до дня `due_date` (включительно);
- первая отметка — день создания, дальше +шаг, пока не выйдем за срок;
- день срока показывается как сама задача, остальные дни окна — превью на календаре;
- без `due_date` повторы на календаре не рисуются;
...
Календарь (`/calendar`) грузит свои `/todos/` и `/shift-days/` на видимый месяц;
повторы раскладывает клиент по правилу выше. Смена дня заливается цветом типа смены.

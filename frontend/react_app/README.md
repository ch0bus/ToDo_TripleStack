...
| `/calendar` | Календарь (`?view=day\|week\|month\|year`); доска смен `?calendar=id`; сводка смен; «Задачи месяца» и «События месяца» |
| `/settings` | Профиль, теги, быстрый вход (PIN / биометрия), тема, ссылка на настройки смен |
